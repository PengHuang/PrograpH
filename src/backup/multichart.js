 ...
 *        type => display tpye [REQUIRED]
 *        data => an array [REQUIRED]
 *          first element => x value [REQUIRED]
 *          second element =>  y value [REQUIRED]
 *        group =>  group [OPTIONAL]
 */
function multichart(args){
  var graphID,containerID
  ...
  function createGraph(){
    if(horizontal){
      ...
      y = d3.scale.linear().range([0, height], bars_gap); 
      ...
      xAxis = d3.svg.axis().scale(x).orient("bottom").tickSize(0, 0, 0);
    ...
    graphID = hp.ID_counter += 1
    var svg = d3.select("#"+containerID).append("svg")
        .attr("width",width+margin.left+margin.top)
        .attr("height", height+margin.top+margin.bottom)
        .attr("id","graph_"+graphID)
  ...
  function resize(){
    ...
    var ratio = current_width/(width+margin.left+margin.right)
    svg()
  ...
  function svg(){
    return d3.select("#"+containerID).select("#graph_"+graphID)
  }
